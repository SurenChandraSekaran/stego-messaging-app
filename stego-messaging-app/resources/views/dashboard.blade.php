...
@php
    ...
    // ── Audit trail: last 2 accepted connections ─────────────────────────────
...
@endphp
    ...
    {{-- No $header slot — we build a premium hero inline ──────────────────── --}}
    <div wire:poll.5s class="bg-slate-950 min-h-screen py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            {{-- ════════════════════════════════════════════════════════════
                 SECTION 2 — Cryptographic Stat Cards (live-polling)
            ════════════════════════════════════════════════════════════ --}}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    {{-- Active Connections: existing Livewire component (all bindings preserved) --}}
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-slate-900 border border-slate-800/80 rounded-xl p-5">
                        ...
                        <div class="space-y-3">
                            @foreach([
                                ['label' => 'Encryption',    'value' => 'AES-256-CBC',          'color' => 'emerald', 'ok' => true],
                                ['label' => 'Steganography', 'value' => 'LSB — Blue channel',    'color' => 'blue',    'ok' => true],
                                ['label' => 'Key Scope',     'value' => 'Per-conversation',      'color' => 'violet',  'ok' => true],
                                ['label' => 'Image Format',  'value' => 'PNG lossless only',     'color' => 'slate',   'ok' => true],
                            ] as $proto)
                    ...
                    {{-- Pending Handshake Alert (only shown when requests exist) --}}
                    @if($pendingIncoming->count() > 0)
                    ...
                    {{-- Security Audit Trail ─── terminal card --}}
                    <div class="bg-slate-900 border border-slate-800/80 rounded-xl p-5">
                        ...
                        <div class="bg-slate-950/60 border border-slate-800/60 rounded-lg p-3 space-y-1.5 font-mono text-[11px] leading-relaxed overflow-hidden">
                            ...
                            {{-- Dynamic entries from real data --}}
                            ...
                            {{-- Static cryptographic system entries --}}
                            <div class="flex gap-2">
